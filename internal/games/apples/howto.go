...
func (g *Game) howtoView(sheet bool) howtoView {
	...
	view := howtoView{
		...
		Timers:     s.AutoDrawSec > 0 || s.SubmitSec > 0 || s.BetweenRevealSec > 0 || s.JudgePickSec > 0,
	}
	g.mu.Lock()
	if g.match != nil {
		ms := g.match.Settings
		...
		view.Timers = ms.AutoDrawSec > 0 || ms.SubmitSec > 0 || ms.BetweenRevealSec > 0 || ms.JudgePickSec > 0
	}
	g.mu.Unlock()
	return view
}
